...
public class ArrayHeap<T> {
    ...
    private int min(int index1, int index2) {
        Node n1 = contents.get(index1);
        Node n2 = contents.get(index2);
        if (n1!=null && n2==null){
            return index1;
        }
        ...
        else if (n1._priority<n2._priority){
            return index1;
        }
        return index2;
    }
    ...
    private void bubbleDown(int index) {
        if (index<contents.size()-1) {
            ...
            if (index != min(index, left) || index != min(index, right)){
    ...
    public void insert(T item, double priority) {
        contents.add(0, new Node(item, priority));
        bubbleDown(0);
    }
    ...
    public void changePriority(T item, double priority) {
        for (int j = 0; j<contents.size(); j++){
            Node n = contents.get(j);
            if (n != null && n._item.equals(item)){
